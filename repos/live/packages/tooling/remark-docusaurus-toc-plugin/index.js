...
const plugin = (options = {}) => {
  ...
  const transformer = node => {
    ...
    let config = {}
    if (firstComment && firstComment.value.startsWith('json')) {
      try {
        config = require('json5').parse(firstComment.value)
      } catch (e) {
        // NOTE: There might not be frontmatter.
        //console.error('Could not parse frontmatter in:', node)
        //console.error('See:', __dirname)
        throw e
      }
    }
    // --

    const headings = search(node, config)
    const {children} = node
